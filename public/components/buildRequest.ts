...
function buildFrom(current: any, resultsPerPage: any) {
  if (!current || !resultsPerPage) return;
  return (current - 1) * resultsPerPage;
}

function buildSort(sortDirection: any, sortField: any) {
  if (sortDirection && sortField) {
    const notKeyword = ['visitors', 'square_km', 'acres', 'date_established'];
    if (!notKeyword.includes(sortField)) {
...
function buildMatch(searchTerm: any) {
  return searchTerm
    ? {
        multi_match: {
          query: searchTerm,
...
*/
export function buildRequest(state: any) {
