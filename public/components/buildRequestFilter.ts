...
function getTermFilter(filter: any) {
  if (filter.type === 'any') {
    return {
      bool: {
        should: filter.values.map((filterValue: any) => ({
    ...
    return {
      bool: {
        filter: filter.values.map((filterValue: any) => ({
          term: getTermFilterValue(filter.field, filterValue),
        })),
      },
    };
...
function getRangeFilter(filter: any) {
  if (filter.type === 'any') {
    return {
      bool: {
        should: filter.values.map((filterValue: any) => ({
...
export function buildRequestFilter(filters: any) {
  if (!filters) return;
